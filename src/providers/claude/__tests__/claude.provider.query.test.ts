...
describe('ClaudeProvider', () => {
  describe('query', () => {
    ...
    it('should capture thinking blocks in trace', async () => {
      ...
      expect(result.trace.turns[0].assistantBlocks[0].text).toBe('Let me analyze this...')
      expect(result.trace.turns[0].assistantBlocks[1].type).toBe('text')
    })

    it('should capture tool_use blocks in trace', async () => {
      const messages: MockSDKMessage[] = [
        assistantMsg([toolUseBlock('call_123', 'search', { query: 'test' })]),
      ...
      expect(result.trace.turns[0].assistantBlocks[0].toolUse?.id).toBe('call_123')
      expect(result.trace.turns[0].assistantBlocks[0].toolUse?.name).toBe('search')
    ...
    it('should map error_max_turns to max_turns termination reason', async () => {
      ...
      await expect(provider.query({ prompt: 'Test' })).rejects.toThrow(AIProviderError)
    })
    ...
    it('should throw AIProviderError on SDK error', async () => {
      const messages: MockSDKMessage[] = [
        errorResult('error_during_execution', ['API error occurred', 'Rate limit exceeded']),
      ]
    ...
    it('should redact sensitive data in trace', async () => {
      const messages: MockSDKMessage[] = [
        assistantMsg([textBlock('Your API key is sk-ant-api03-test123456789012345')]),
        successResult(),
      ]
    ...
    it('should concatenate text from multiple text blocks', async () => {
      const messages: MockSDKMessage[] = [
        assistantMsg([textBlock('Part one. '), textBlock('Part two.')]),
        successResult(),
      ]
    ...
    it('should use SDK result text for multi-turn responses instead of accumulated assistant text', async () => {
      ...
      expect(result.content).toBe('Final multi-turn response with the actual joke about files.')
    ...
    it('should capture user message tool_use_result into currentTurn.toolResults', async () => {
      const messages: MockSDKMessage[] = [
        assistantMsg([toolUseBlock('call_bash_1', 'Bash', { command: 'ls' })]),
    ...
    it('should resolve toolUseId from parent_tool_use_id when available', async () => {
      const messages: MockSDKMessage[] = [
        assistantMsg([toolUseBlock('call_read_1', 'Read', { file_path: '/foo' })]),
    ...
    it('should fall back to last tool_use block id when parent_tool_use_id is null', async () => {
      const messages: MockSDKMessage[] = [
        assistantMsg([toolUseBlock('call_glob_1', 'Glob', { pattern: '*.ts' })]),
    ...
    it('should handle string tool_use_result content', async () => {
      const messages: MockSDKMessage[] = [
        assistantMsg([toolUseBlock('call_1', 'Read', { file_path: '/test' })]),

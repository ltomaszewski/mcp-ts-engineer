...
export const PhaseFileSchema = z.object({
  ...
  purpose: z
    .string()
    .describe("Brief description of file's role in this phase"),
});
...
export const PhaseSchema = z.object({
  ...
  dependencies: z
    .array(z.string())
    .describe("Phase numbers or 'none' if no dependencies"),
...
export const PhaseEngResultSchema = z.object({
  ...
  files_modified: z
    .array(z.string())
    .describe("Actual files changed during engineering"),
...
export const PhaseAuditResultSchema = z.object({
  status: z
    .enum(["pass", "warn", "fail"])
    .describe("Audit status for this phase"),
...
export const PhaseStatusSchema = z.object({
  ...
  audit_status: z
    ...
    .describe("Audit step execution status"),
  files_modified: z
    .array(z.string())
    .describe("Files modified during this phase"),
  retry_attempts: z
    .number()
    .int()
    .min(0)
    .describe("Number of retry attempts for this phase"),
});
...
export const PhaseEngStepInputSchema = z.object({
  ...
  current_phase_number: z
    .number()
    .int()
    .min(1)
    .describe("Current phase number to implement"),
  cwd: z
    .string()
    .optional()
...
export const PhaseAuditStepInputSchema = z.object({
  ...
  files_modified: z
    .array(z.string())
    .describe("Files modified during engineering step"),
...
export const FinalAuditStepInputSchema = z.object({
  spec_path: z
    ...
    .min(1, "spec_path is required")
    .describe("Path to the spec file"),
  all_modified_files: z
    .array(z.string())
    .describe("All files modified across all phases"),
  cwd: z
    .string()
    .optional()
    .describe("Working directory for tool execution"),
...
export const CommitStepInputSchema = z.object({
  ...
  phase_summaries: z
    .array(z.string())
    .describe("Summaries from each phase"),
  final_audit_summary: z
    .string()
    .describe("Summary from final audit"),
  cwd: z
    .string()
    .optional()
    .describe("Working directory for tool execution"),
...
export const TodoCodeWriterOutputSchema = z.object({
  ...
  phases_completed: z
    ...
    .min(0)
    .max(10)
    .describe("Number of phases actually completed"),
  final_audit_status: z
    .enum(["pass", "warn", "fail"])
    .describe("Final audit result"),
  commit_sha: z
    .string()
    .nullable()
  ...
  files_changed: z
    .array(z.string())
    .describe("List of files changed across all phases"),
  ...
  phase_results: z
    .array(PhaseStatusSchema)
    .describe("Detailed status for each phase executed"),
});

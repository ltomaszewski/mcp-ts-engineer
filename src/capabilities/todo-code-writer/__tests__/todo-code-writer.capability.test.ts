...
const VALID_PHASE_PLAN = {
  phases: [
    ...
    },
    {
      ...
      dependencies: ["1"],
      files: [
        { path: "src/helpers.ts", action: "CREATE" as const, purpose: "Utility functions" },
      ],
    },
  ],
};
...
describe("todoCodeWriterCapability", () => {
  ...
  describe("default request options", () => {
    ...
    it("defaults to sonnet model", () => {
      expect(todoCodeWriterCapability.defaultRequestOptions?.model).toBe("sonnet");
    });
    ...
    it("uses bypassPermissions mode", () => {
      expect(todoCodeWriterCapability.defaultRequestOptions?.permissionMode).toBe("bypassPermissions");
    });

    it("allows dangerously skip permissions", () => {
      expect(todoCodeWriterCapability.defaultRequestOptions?.allowDangerouslySkipPermissions).toBe(true);
    });
  });
  ...
  describe("processResult orchestration", () => {
    ...
    it("parses phase plan and orchestrates eng→audit per phase, then final audit and commit", async () => {
      ...
      mockInvoke
        ...
        .mockResolvedValueOnce({ ...VALID_ENG_RESULT, files_modified: ["src/helpers.ts"], summary: "Helpers done" }) // Phase 2 eng
      ...
      expect(mockInvoke).toHaveBeenNthCalledWith(1, "todo_code_writer_phase_eng_step", expect.objectContaining({
        spec_path: "docs/specs/feature.md",
        phase_plan: VALID_PHASE_PLAN,
        current_phase_number: 1,
      }));
      expect(mockInvoke).toHaveBeenNthCalledWith(2, "todo_code_writer_phase_audit_step", expect.objectContaining({
        spec_path: "docs/specs/feature.md",
        phase_number: 1,
      }));
      expect(mockInvoke).toHaveBeenNthCalledWith(5, "todo_code_writer_final_audit_step", expect.anything());
      expect(mockInvoke).toHaveBeenNthCalledWith(6, "todo_code_writer_commit_step", expect.anything());
    });
    ...
    it("deduplicates modified files across phases", async () => {
      ...
      mockInvoke
        .mockResolvedValueOnce({ ...VALID_ENG_RESULT, files_modified: ["src/schemas.ts", "src/common.ts"] })
        .mockResolvedValueOnce(VALID_AUDIT_RESULT)
        .mockResolvedValueOnce({ ...VALID_ENG_RESULT, files_modified: ["src/common.ts", "src/helpers.ts"] }) // Duplicate src/common.ts
        .mockResolvedValueOnce(VALID_AUDIT_RESULT)
        .mockResolvedValueOnce(VALID_FINAL_AUDIT_RESULT)
        .mockResolvedValueOnce(VALID_COMMIT_RESULT);
      ...
      const finalAuditCall = mockInvoke.mock.calls.find(call => call[0] === "todo_code_writer_final_audit_step");
    ...
    it("passes phase summaries to commit step", async () => {
      ...
      const commitCall = mockInvoke.mock.calls.find(call => call[0] === "todo_code_writer_commit_step");
  ...
  describe("retry and halt logic", () => {
    ...
    it("excludes failed phase files from commit file list", async () => {
      ...
      mockInvoke
        ...
        .mockResolvedValueOnce(VALID_AUDIT_RESULT)
        .mockResolvedValueOnce({ ...VALID_ENG_RESULT, files_modified: ["src/helpers.ts"], status: "failed" })
        .mockResolvedValueOnce(VALID_FINAL_AUDIT_RESULT)

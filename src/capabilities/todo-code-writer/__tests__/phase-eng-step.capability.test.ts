...
describe('phaseEngStepCapability', () => {
  describe('definition metadata', () => {
    it('has correct id', () => {
      expect(phaseEngStepCapability.id).toBe('todo_code_writer_phase_eng_step')
    })
    ...
    it('defaults to sonnet model', () => {
      expect(phaseEngStepCapability.defaultRequestOptions?.model).toBe('sonnet')
    })
    ...
    it('has outputSchema configured', () => {
      expect(phaseEngStepCapability.defaultRequestOptions?.outputSchema).toBeDefined()
    })

    it('has appendSystemPrompt set to undefined (lazy-loaded at runtime via buildDevContext)', () => {
      const append = phaseEngStepCapability.defaultRequestOptions?.appendSystemPrompt
    ...
    it('includes path validation hooks in defaultRequestOptions', () => {
      ...
      const hooks = phaseEngStepCapability.defaultRequestOptions?.hooks as unknown as {
        PreToolUse?: unknown[]
      }
  ...
  describe('preparePromptInput', () => {
    ...
    function getPromptInput(input: Record<string, unknown>): Record<string, unknown> {
      ...
      return phaseEngStepCapability.preparePromptInput(input as never, context as never) as Record<
        string,
        unknown
      >
    }

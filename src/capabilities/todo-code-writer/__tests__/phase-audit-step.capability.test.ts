...
describe("phaseAuditStepCapability", () => {
  describe("definition metadata", () => {
    it("has correct id", () => {
      expect(phaseAuditStepCapability.id).toBe(
    ...
    it("defaults to sonnet model", () => {
      expect(phaseAuditStepCapability.defaultRequestOptions?.model).toBe(
        "sonnet",
      );
    });
    ...
    it("defaults to $2.0 budget", () => {
      expect(phaseAuditStepCapability.defaultRequestOptions?.maxBudgetUsd).toBe(
        2.0,
      );
    });
    ...
    it("has outputSchema configured", () => {
      expect(
        phaseAuditStepCapability.defaultRequestOptions?.outputSchema,
      ).toBeDefined();
    });
    ...
    it("prompt v1 systemPrompt.append contains review context", () => {
      ...
      const append =
        (prompt.systemPrompt as { append?: string }).append ?? "";
    ...
    it("includes path validation hooks in defaultRequestOptions", () => {
      ...
      const hooks = phaseAuditStepCapability.defaultRequestOptions?.hooks as unknown as { PreToolUse?: unknown[] };
  ...
  describe("v2 prompt skill loading", () => {
    ...
    it("v2 prompt includes LOAD SKILLS FIRST rule", () => {
      ...
      expect(prompt.userPrompt).toContain(
        "1. Load all skills listed in <skill_loading>",
      );
    });

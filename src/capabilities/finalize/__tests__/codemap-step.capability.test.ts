...
describe("finalizeCodemapStepCapability", () => {
  describe("definition metadata", () => {
    ...
    it("has bypassPermissions enabled", () => {
      expect(finalizeCodemapStepCapability.defaultRequestOptions?.permissionMode).toBe("bypassPermissions");
    });

    it("has allowDangerouslySkipPermissions enabled", () => {
      expect(finalizeCodemapStepCapability.defaultRequestOptions?.allowDangerouslySkipPermissions).toBe(true);
    });
  ...
  describe("processResult", () => {
    ...
    it("handles multiple codemaps changed", async () => {
      const codemapResult = {
        updated: true,
        codemaps_changed: [
          ".claude/codemaps/feature1.md",
          ".claude/codemaps/feature2.md",
        ],

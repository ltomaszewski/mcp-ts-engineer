...
describe("finalizeCommitStepCapability", () => {
  describe("definition metadata", () => {
    ...
    it("has bypassPermissions enabled", () => {
      expect(finalizeCommitStepCapability.defaultRequestOptions?.permissionMode).toBe("bypassPermissions");
    });

    it("has allowDangerouslySkipPermissions enabled", () => {
      expect(finalizeCommitStepCapability.defaultRequestOptions?.allowDangerouslySkipPermissions).toBe(true);
    });
  ...
  describe("preparePromptInput", () => {
    ...
    it("includes sessionId from context.session.id", () => {
      ...
      const result = finalizeCommitStepCapability.preparePromptInput(input, context) as Record<string, unknown>;

...
describe("finalizeCapability", () => {
  ...
  describe("processResult orchestration", () => {
    ...
    it("skips test step when skip_tests=true", async () => {
      ...
      expect(context.invokeCapability).toHaveBeenCalledWith("finalize_audit_step", expect.any(Object));
      expect(context.invokeCapability).not.toHaveBeenCalledWith("finalize_test_step", expect.any(Object));
      expect(context.invokeCapability).toHaveBeenCalledWith("finalize_codemap_step", expect.any(Object));
      expect(context.invokeCapability).toHaveBeenCalledWith("finalize_commit_step", expect.any(Object));
    ...
    it("skips codemap step when skip_codemaps=true", async () => {
      ...
      expect(context.invokeCapability).toHaveBeenCalledWith("finalize_audit_step", expect.any(Object));
      expect(context.invokeCapability).toHaveBeenCalledWith("finalize_test_step", expect.any(Object));
      expect(context.invokeCapability).not.toHaveBeenCalledWith("finalize_codemap_step", expect.any(Object));
      expect(context.invokeCapability).toHaveBeenCalledWith("finalize_commit_step", expect.any(Object));
  ...
  describe("finalize plan parsing", () => {
    ...
    it("falls back to empty arrays when finalize_plan contains invalid JSON", async () => {
      ...
      const aiResult = createMockAiResult(
        `<finalize_plan>{ invalid json }</finalize_plan>`
      );

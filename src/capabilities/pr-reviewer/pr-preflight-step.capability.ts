...
import { sanitizePrContext, tryParseJson } from './pr-reviewer.helpers.js'
...
> = {
  ...
  processResult: (
    ...
    const FALLBACK: PreflightStepOutput = {
      proceed: false,
      skip_reason: 'Failed to parse AI result',
    }
    ...
    if (aiResult.structuredOutput) {
      ...
      if (validated.success) {
        ...
        if (validated.data.proceed && !prCtx) {
          return {
            proceed: false,
            skip_reason: 'Failed to validate pr_context from structured output',
          }
        }

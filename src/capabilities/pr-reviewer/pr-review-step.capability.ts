...
export const prReviewStepCapability: CapabilityDefinition<
  ReviewStepInput,
  ReviewStepOutput
> = {
  id: "pr_review_step",
  ...
  processResult: (
    ...
    if (aiResult.structuredOutput) {
      ...
      if (validated.success) {
        _context.logger.info("Review parsed via structured output", { issueCount: validated.data.issues.length });
    ...
    if (parsed?.issues && Array.isArray(parsed.issues)) {
      _context.logger.info("Review parsed via regex JSON extraction", { issueCount: parsed.issues.length });
      return parsed;
    }

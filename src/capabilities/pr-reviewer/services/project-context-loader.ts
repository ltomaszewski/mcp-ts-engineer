...
const SKILL_DETECTION_RULES: SkillDetectionRule[] = [
  {
    matches: (f) => f.endsWith(".module.ts") || f.endsWith(".service.ts") || f.endsWith(".controller.ts"),
    skill: "nestjs-core",
  },
  {
  ...
  {
    matches: (f) => f.endsWith(".resolver.ts") || f.includes("/graphql/") || f.includes("\\graphql\\"),
    skill: "nestjs-graphql",
  },
  {
    matches: (f) => f.endsWith(".tsx") || f.includes("/expo/") || f.includes("\\expo\\") || f.includes("react-native"),
    skill: "expo-core",
  },
  {

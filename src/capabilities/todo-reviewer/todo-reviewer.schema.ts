...
export const ReviewSummarySchema = z.object({
  status: z
    ...
    .describe(
      'Spec review status: READY (implementation complete), IN_REVIEW (reviewed, awaiting implementation), or BLOCKED',
    ),
  spec_path: z.string().describe('Path to the reviewed spec file'),
  target_app: z.string().describe('Target app or package the spec applies to'),
  corrections_applied: z.number().int().describe('Number of corrections applied to the spec'),
  blockers_remaining: z.number().int().describe('Number of unresolved blockers'),
  warnings: z.number().int().describe('Number of warnings found'),
  cross_app_status: z
    .enum(['READY', 'BLOCKED', 'N/A'])
    .describe('Cross-app consistency check status'),
  consistency_score: z.string().describe("Human-readable consistency score (e.g. '8/10')"),
  key_findings: z.array(z.string()).describe('Summary of key findings from the review'),
  spec_modified: z.boolean().describe('Whether the spec file was modified during review'),
})
...
export const TddSummarySchema = z.object({
  status: z.enum(['PASS', 'FAIL', 'WARN']).describe('Overall TDD validation result'),
  details: z.string().describe('Human-readable explanation of the result'),
  issues_found: z.number().int().describe('Number of issues detected during validation'),
  spec_modified: z.boolean().describe('Whether the spec was modified to fix TDD issues'),
})
...
export const TddIssueSchema = z.object({
  ...
  file_path: z.string().optional().describe('Affected file path if applicable'),
...
export const ScopeAnalysisSchema = z.object({
  files_changed: z.number().int().min(0).describe("Count from 'Files Changed' section"),
  tests_defined: z.number().int().min(0).describe('Total test files in spec'),
  tests_in_scope: z.number().int().min(0).describe('Tests targeting changed files'),
  tests_out_of_scope: z.number().int().min(0).describe('Tests targeting unchanged files'),
  justified_regression: z
    .number()
    .int()
    .min(0)
    .describe('Out-of-scope with documented justification'),
  unjustified_scope_creep: z.number().int().min(0).describe('Out-of-scope without justification'),
  scope_verdict: z
...
export const CoverageAnalysisSchema = z.object({
  coverage_target: z.string().describe("Coverage target (e.g. '80%') or 'missing'"),
  coverage_explicit: z.boolean().describe('Target is explicitly stated in spec'),
  fr_ec_total: z.number().int().min(0).describe('Total FR/EC count'),
  fr_ec_with_tests: z.number().int().min(0).describe('FR/EC with mapped tests'),
  forward_traceability: z.enum(['complete', 'gaps']).describe('FR/EC → tests traceability'),
  ...
  test_scenarios: z.object({
    ...
    error_conditions: z.number().int().min(0).describe('Error condition test count'),
  }),
  yagni_violations: z.number().int().min(0).describe('Tests for library/framework internals'),
})
...
export const TddScanStepResultSchema = z.object({
  status: z.enum(['PASS', 'FAIL', 'WARN']).describe('Overall TDD scan result'),
  scope_analysis: ScopeAnalysisSchema.describe('Scope boundary analysis'),
  coverage_analysis: CoverageAnalysisSchema.describe('Coverage adequacy analysis'),
  issues: z.array(TddIssueSchema).describe('List of detected issues'),
  spec_modified: z.boolean().describe('Whether scan made any corrections'),
  needs_fix: z.boolean().describe('True if CRITICAL or HIGH issues exist'),
...
export const TddScanStepInputSchema = z.object({
  spec_path: z.string().min(1, 'spec_path is required').describe('Path to the spec file to scan'),
  review_summary: ReviewSummarySchema.describe('Review summary from the preceding review step'),
...
export const TddFixStepInputSchema = z.object({
  spec_path: z.string().min(1, 'spec_path is required').describe('Path to the spec file to fix'),
  scan_result: TddScanStepResultSchema.describe('Scan result from the TDD scan step'),
  cwd: z.string().optional().describe('Working directory for tool execution'),
})
...
export const TddFixStepResultSchema = z.object({
  status: z.enum(['success', 'partial', 'failed']).describe('Fix operation status'),
  issues_fixed: z.number().int().min(0).describe('Number of issues fixed'),
  issues_remaining: z.number().int().min(0).describe('Number of issues remaining'),
...
export const CommitStepInputSchema = z.object({
  ...
  review_summary: ReviewSummarySchema.describe('Review summary from the review step'),
  tdd_summary: TddSummarySchema.describe('TDD summary from the validation step'),
  cwd: z.string().optional().describe('Working directory for tool execution'),
})
...
export const TodoReviewerInputSchema = z.object({
  ...
  model: z.enum(['opus', 'sonnet']).default('sonnet').describe('Model to use for the review'),
  iterations: z
    .number()
    .int()
    .min(1)
...
export const TodoReviewerOutputSchema = z.object({
  ...
  review_report: z.string().describe('Human-readable review report from the last iteration'),
  tdd_report: z.string().describe('Human-readable TDD validation report from the last iteration'),
  ...
  files_changed: z.array(z.string()).describe('List of files changed across all iterations'),
  session_id: z
    .string()
    .optional()

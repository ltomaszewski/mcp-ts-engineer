...
import { todoReviewerCapability } from '../todo-reviewer.capability.js'
...
describe('todoReviewerCapability', () => {
  ...
  describe('processResult (single iteration)', () => {
    ...
    it('calls invokeCapability for tdd_validate and commit steps', async () => {
      ...
      mockInvoke.mockResolvedValueOnce(VALID_SCAN_RESULT).mockResolvedValueOnce(VALID_COMMIT_RESULT)
...
\`\`\`
      ...
      expect(updatedContent).toMatch(
        /\|\s+apps\/my-app\/src\/modules\/user\/user\.service\.ts\s+\|/,
      )
...
\`\`\`
      ...
      await expect(todoReviewerCapability.processResult(input, aiResult, context)).rejects.toThrow(
        /Invalid paths in spec:/,
      )

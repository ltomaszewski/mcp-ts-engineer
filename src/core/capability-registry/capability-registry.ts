...
import type { CapabilityDefinition, CapabilityType } from './capability-registry.types.js'
...
export class CapabilityRegistry {
  ...
  registerCapability(definition: CapabilityDefinition): void {
    if (this.capabilities.has(definition.id)) {
      throw new CapabilityError(`Capability ${definition.id} already registered`)
    }
  ...
  bindToMcpServer(server: McpServer): void {
    const tools = this.listCapabilities('tool').filter((cap) => cap.visibility !== 'internal')

 ...
 */
function createLoggerProxy(baseLogger: ReturnType<typeof import("../logger/index.js").Logger.prototype.withContext>): CapabilityContext["logger"] {
  return {
...
function createSessionCostGetter(
  ...
  sessionId: string
): () => { totalCostUsd: number; totalInputTokens: number; totalOutputTokens: number; totalTurns: number } {
  return () => {
...
function propagateChildCost(
  ...
  if (parsed && typeof parsed === "object" && !Array.isArray(parsed)) {
    ...
    const totalTokensIn = typeof inputTokens === "number" && typeof cacheRead === "number"
    ...
    if (
      ...
      deps.costTracker.recordChildCost(sessionId, invocationId, childCapabilityName, {
        ...
        ...(typeof cacheWrite === "number" && cacheWrite > 0 ? { promptCacheWrite: cacheWrite } : {}),
        ...(typeof cacheRead === "number" && cacheRead > 0 ? { promptCacheRead: cacheRead } : {}),
        ...(typeof promptVer === "string" && promptVer.length > 0 ? { promptVersion: promptVer } : {}),

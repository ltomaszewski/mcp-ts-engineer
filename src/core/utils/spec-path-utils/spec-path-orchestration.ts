...
export async function validateAndCorrectSpecPaths(
  ...
  if (correctionResult.corrections.length > 0) {
    ...
    writeFileSync(join(workingDir, specPath), correctionResult.correctedContent)
  ...
  if (filesystemCorrections.length > 0) {
    ...
    remaining = remaining.filter((p) => !filesystemCorrections.some((c) => c.original === p))
  }
  ...
  try {
    ...
    if (aiResult.status === 'SUCCESS' || aiResult.status === 'PARTIAL') {
      ...
      if (aiResult.corrected_content.length > 0) {
        writeFileSync(join(workingDir, specPath), aiResult.corrected_content)
      }

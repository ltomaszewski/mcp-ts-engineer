...
describe('classifyPath', () => {
  ...
  describe('internal directory paths', () => {
    ...
    it('recognizes capabilities/echo-agent/echo.capability.ts as correctable', () => {
      expect(classifyPath('capabilities/echo-agent/echo.capability.ts')).toBe('correctable')
    })
...
describe('correctPath', () => {
  ...
  describe('internal directory corrections', () => {
    it('transforms core/foo.ts to apps/mcp-ts-engineer/src/core/foo.ts', () => {
      expect(correctPath('core/foo.ts', target)).toBe('apps/mcp-ts-engineer/src/core/foo.ts')
    })

    it('transforms lib/bar.ts to apps/mcp-ts-engineer/src/lib/bar.ts', () => {
      expect(correctPath('lib/bar.ts', target)).toBe('apps/mcp-ts-engineer/src/lib/bar.ts')
    })
  ...
  describe('existing correction patterns', () => {
    it('transforms src/foo.ts to apps/mcp-ts-engineer/src/foo.ts', () => {
      expect(correctPath('src/foo.ts', target)).toBe('apps/mcp-ts-engineer/src/foo.ts')
    })

    it('transforms ./foo.ts to apps/mcp-ts-engineer/foo.ts', () => {
      expect(correctPath('./foo.ts', target)).toBe('apps/mcp-ts-engineer/foo.ts')
    })
  })

  describe('no correction needed', () => {
    it('leaves valid monorepo paths unchanged', () => {
      expect(correctPath('apps/my-server/src/main.ts', target)).toBe('apps/my-server/src/main.ts')
    })

    it('leaves docs/ paths unchanged', () => {
      expect(correctPath('docs/specs/foo.md', target)).toBe('docs/specs/foo.md')
    })

    it('leaves .claude/ paths unchanged', () => {
      expect(correctPath('.claude/rules/bar.md', target)).toBe('.claude/rules/bar.md')
    })
  })
})

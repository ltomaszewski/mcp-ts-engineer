...
const { validateAndCorrectSpecPaths } = await import('../spec-path-orchestration.js')
...
describe('validateAndCorrectSpecPaths', () => {
  ...
  describe('Tier 1: Deterministic correction', () => {
    it('corrects src/ paths and writes to file', async () => {
      ...
      const correctedContent = 'Check `apps/test-app/src/components/Button.tsx` file.'

      mockReadFileSync.mockReturnValue(specContent)
      mockWriteFileSync.mockImplementation(() => {})

      await validateAndCorrectSpecPaths('docs/specs/test/feature.md', 'test-app', mockContext)
  ...
  describe('Tier 2: Filesystem check', () => {
    ...
    it('skips correction when file does not exist', async () => {
      ...
      await expect(
        validateAndCorrectSpecPaths('docs/specs/test/feature.md', 'test-app', mockContext),
  ...
  describe('edge cases', () => {
    it('throws ValidationError when spec file cannot be read', async () => {
      ...
      await expect(
        validateAndCorrectSpecPaths('nonexistent/spec.md', 'test-app', mockContext),
      ).rejects.toThrow(ValidationError)
      await expect(
        validateAndCorrectSpecPaths('nonexistent/spec.md', 'test-app', mockContext),

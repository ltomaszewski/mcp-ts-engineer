...
function createMockContext(): CapabilityContext {
  return {
    ...
    invokeCapability: vi
      .fn<(capabilityName: string, input: unknown) => Promise<unknown>>()
      .mockResolvedValue({}),
  }
}

// Helper to create mock AI result
function createMockAiResult(content: string, structuredOutput?: PathFixStepOutput): AIQueryResult {
  return {
    content,
...
describe('pathFixStepCapability', () => {
  ...
  describe('defaultRequestOptions', () => {
    ...
    it('sets maxBudgetUsd to $0.50', () => {
      expect(pathFixStepCapability.defaultRequestOptions?.maxBudgetUsd).toBe(0.5)
    })
    ...
    it('uses bypassPermissions for autonomous operation', () => {
      expect(pathFixStepCapability.defaultRequestOptions?.permissionMode).toBe('bypassPermissions')
      expect(pathFixStepCapability.defaultRequestOptions?.allowDangerouslySkipPermissions).toBe(
        true,
      )
    })

    it('includes outputSchema for structured output', () => {
      expect(pathFixStepCapability.defaultRequestOptions?.outputSchema).toBeDefined()
      expect(pathFixStepCapability.defaultRequestOptions?.outputSchema).toHaveProperty(
        'type',
        'json_schema',
      )
    })
  ...
  describe('preparePromptInput', () => {
    it('extracts correct fields from PathFixStepInput', () => {
      ...
      const result = pathFixStepCapability.preparePromptInput(input, mockContext)
    ...
    it('handles optional cwd field', () => {
      ...
      const mockContext = createMockContext()
      const result = pathFixStepCapability.preparePromptInput(input, mockContext) as {
        specPath: string
        specContent: string
        targetApp: string
  ...
  describe('processResult', () => {
    ...
    it('parses structured output successfully', () => {
      ...
      const mockAiResult = createMockAiResult('Some text response', validOutput)
    ...
    it('returns fallback on parse failure', () => {
      ...
      const mockAiResult = createMockAiResult('Some text response', invalidStructuredOutput as any)
    ...
    it('returns fallback when no structured output or XML', () => {
      const mockAiResult = createMockAiResult('Plain text response without XML tags')
      ...
      expect(result.corrections).toEqual([])
      expect(result.remaining_uncorrectable).toEqual([])
      expect(result.corrected_content).toBe(mockAiResult.content.slice(0, 500))
    })
  ...
  describe('inputSchema validation', () => {
    it('accepts valid input', () => {
      ...
      const result = pathFixStepCapability.inputSchema.safeParse(validInput)
    ...
    it('rejects empty spec_path', () => {
      ...
      const result = pathFixStepCapability.inputSchema.safeParse(invalidInput)

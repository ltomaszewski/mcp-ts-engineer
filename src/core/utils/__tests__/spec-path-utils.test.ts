...
import { correctSpecPaths, extractFilePaths, validateSpecPaths } from '../spec-path-utils.js'
...
| CREATE | apps/my-server/src/modules/auth/auth.service.ts | Auth service |
      ...
      const paths = extractFilePaths(content)
      expect(paths).toContain('apps/my-server/src/modules/auth/auth.service.ts')
    })
  })

  describe('inline code', () => {
    it('extracts paths from inline code blocks', () => {
      const content = 'Update the file `apps/my-app/src/config.ts` to include the new settings.'
    ...
    it('extracts multiple inline paths', () => {
      const content = 'Import from `packages/utils/src/date.ts` and `packages/types/src/index.ts`.'
...
\`\`\`
    ...
    it('ignores node_modules paths', () => {
      const content = 'The file is at `node_modules/react/index.js` in the cache.'
...
- \`apps/my-app/src/config.ts\`
      ...
      const paths = extractFilePaths(content)
      expect(paths.filter((p: string) => p === 'apps/my-app/src/config.ts')).toHaveLength(1)
    })
  })
  ...
  describe('false positive filtering', () => {
    ...
    describe('extension whitelist', () => {
      it('extracts paths with valid extensions', () => {
        const content = '`apps/my-app/src/file.ts` and `packages/types/index.json`'
        const paths = extractFilePaths(content)
...
describe('validateSpecPaths', () => {
  describe('valid paths', () => {
    it('returns valid for paths starting with apps/', () => {
      const paths = ['apps/my-app/src/components/Button.tsx', 'apps/my-server/src/auth.ts']

      const result = validateSpecPaths(paths, 'my-app')

      expect(result.valid).toContain('apps/my-app/src/components/Button.tsx')
    ...
    it('returns valid for paths starting with packages/', () => {
      const paths = ['packages/types/src/models/user.ts', 'packages/utils/src/format.ts']
  ...
  describe('uncorrectable paths', () => {
    it('returns uncorrectable for absolute paths', () => {
      const paths = ['/Users/dev/project/src/file.ts', '/home/user/app/config.ts']
...
\`\`\`
      ...
      expect(result.correctedContent).toContain('apps/my-app/src/components/Button.tsx')
...
| src/user.ts | UPDATE |
      ...
      expect(result.correctedContent).toContain('apps/my-server/src/auth.ts')
      expect(result.correctedContent).toContain('apps/my-server/src/user.ts')
  ...
  describe('correcting ./ paths', () => {
    it('corrects ./foo.ts to apps/{target}/foo.ts', () => {
      ...
      expect(result.correctedContent).toContain('apps/my-app/utils/helper.ts')
      expect(result.corrections).toContainEqual({
        original: './config.ts',
...
- \`/Users/dev/uncorrectable.ts\`
      ...
      expect(result.correctedContent).toContain('apps/my-app/src/valid.ts')
      expect(result.correctedContent).toContain('apps/my-app/src/already-valid.ts')
